...
def save_to_file(entries, filename='entries.txt'):
    with open(filename, 'w') as file:
        for entry in entries:
            file.write(f"{entry}\n")

def load_from_file(filename='entries.txt'):
    entries = []
    try:
...
if __name__ == "__main__":
    ...
    autocompleteList = load_from_file()

    def on_submit(event):
        # Add the entry to the autocomplete list and save:
        entry_text = entry.get()
        if entry_text and entry_text not in autocompleteList:
            autocompleteList.append(entry_text)
            save_to_file(autocompleteList)
    ...
    entry.bind('<Return>', on_submit)  # Bind the Return key to on_submit
