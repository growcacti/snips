...
ITEM_TYPE = {
    "content": ["Treasure Chest", "On Event"],
    "start": (140, 400),
    "space": (0, 20),
    "size": (100, 20),
...
SPECIAL_TYPE = {
    "content": ["Portal", "Push", "Burnable", "Breakable"],
    "start": (140, 50),
    "space": (0, 20),
    "size": (100, 20),
    "selected": "Push",
...
class Panel(object):
    ...
    def pull_rect(self):
        ...
        return PULL_TAB.get_rect(right=self.rect.right - 2, centery=self.rect.centery)

    @property
    def arrow_rect(self):
        """Conveniece for retrieving the position of the pull tab arrows."""
        pull_rect = self.pull_rect
        return ARROWS.get_rect(centerx=pull_rect.centerx - 1, centery=pull_rect.centery)
