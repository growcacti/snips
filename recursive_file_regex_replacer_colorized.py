    ...
    for path, dirs, files in os.walk(os.path.abspath(cfg.dir)):
        for filename in fnmatch.filter(files, cfg.glob):
            ...
            if os.path.islink(full_path):
                print(
                    "{}File {} is a symlink. Skipping{}".format(
                        Colors.Red, full_path, Colors.NoColor
                    )
                )
                continue
            ...
            if all_matches:
                print(
                    "{}Found {} match(es) in file {}{}".format(
                        Colors.LightMagenta, len(all_matches), filename, Colors.NoColor
                    )
                )
                ...
                if cfg.verbose or cfg.dry_run:
                    colorized_old = search_pattern.sub(
                        Colors.LightBlue + r"\g<0>" + Colors.NoColor, old_text
                    )
                    colorized_old = "\n".join(
                        [
                            "\t" + line.strip()
                            for line in colorized_old.split("\n")
                            if Colors.LightBlue in line
                        ]
                    )

                    colorized = search_pattern.sub(
                        Colors.Green + cfg.replace_regex + Colors.NoColor, old_text
                    )
                    colorized = "\n".join(
                        [
                            "\t" + line.strip()
                            for line in colorized.split("\n")
                            if Colors.Green in line
                        ]
                    )
                    print(
                        "{}BEFORE:{}\n{}".format(
                            Colors.White, Colors.NoColor, colorized_old
                        )
                    )
                    print(
                        "{}AFTER :{}\n{}".format(
                            Colors.Yellow, Colors.NoColor, colorized
                        )
                    )

            elif cfg.list_non_matching:
                print(
                    'File {} does not contain search regex "{}"'.format(
                        filename, cfg.search_regex
                    )
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--search-regex", "-s", help="search regex", required=True)

    parser.add_argument(
        "--replace-regex", "-r", help="replacement regex", required=True
    )

    parser.add_argument("--glob", "-g", help="glob pattern, i.e. *.html", default="*.*")
